...
static unsigned int default_efficient_freq_pr[] = {2553600};
static u64 default_up_delay_pr[] = {125 * NSEC_PER_MSEC};
				...
				unsigned int flags)
{
	...
	next_f = get_next_freq(sg_policy, time, sg_cpu->util, max_cap);
...
static unsigned int sugov_next_freq_shared(struct sugov_cpu *sg_cpu, u64 time)
{
	...
	return get_next_freq(sg_policy, time, util, max_cap);
}
									...
									const char *buf, size_t count)
{
	...
	if (new_efficient_freq) {
		...
		if (old != default_efficient_freq_pr)
			kfree(old);
	}
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	if (cpumask_test_cpu(sg_policy->policy->cpu, cpu_prime_mask)) {
